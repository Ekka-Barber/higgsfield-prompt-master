...
name: hf-help
description: >
  Index of all /hf-* commands — what each one does and when to reach for it.
  One-shot reference card, not a mode. Use for /hf-help, "المساعدة", "الأوامر",
  "وش الأوامر", "hf help", "what commands", "list commands", "how do I use hf".
---

# /hf-help — command index
...
| Command | Use it for |
|---|---|
| **`/hf`** | Start here. Grills you, routes, generates. |
| **`/hf-arabic`** | Arabic lettering *is* the design — calligraphy, wordmarks, RTL typography. |
| **`/hf-brief`** | Interrogation only. Produces a locked spec, no prompt. |
| **`/hf-social`** | Instagram, Snap, TikTok, X, LinkedIn, YouTube — ratios & safe zones. |
| **`/hf-poster`** | Posters, flyers, banners, signage, event announcements. |
| **`/hf-menu`** | Restaurants & cafés — dish photography, menu boards, food styling. |
| **`/hf-brand`** | Logos, Arabic wordmarks, monograms, identity systems. |
| **`/hf-product`** | Packshots, hero shots, lifestyle, marketplace listings. |
| **`/hf-edit`** | Edit an existing image — references, face lock, background swap, localisation. |
| **`/hf-search`** | Explore the 7,315-prompt corpus for real exemplars. |
| **`/hf-model`** | Which model, and is my size valid? |
| **`/hf-help`** | This card. |
